...
subroutine backSubstitution(A,b,x,rSq,m,n)
  integer, parameter:: dk = selected_real_kind(15, 307)
...
! subroutine utilizing a LAPACK Householder routine
subroutine hhQRLapack(A,b,m,n)
  integer, parameter:: dk = selected_real_kind(15,307)
  real (kind = dk), dimension(:,:), intent(in):: A
  real (kind = dk), dimension(:,:), intent(out):: b 
  integer:: m, n, lda, info, lwork, nb
  real (kind = dk), dimension(:), allocatable:: work
  ...
  lwork = m+ nb*m
  allocate(work(lwork))
  ! call subroutine DGELS from LAPACK which solves overdetermind least squares
  ! problem using QR factorization. For QR factorization, it uses householder
  ! transformation
  call DGELS('No transpose', m, n, 1, A, m, b, m, work, lwork, info)
  return
end subroutine hhQRLapack
end module houseHolderQR1
